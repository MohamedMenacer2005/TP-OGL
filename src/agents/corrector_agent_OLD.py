...
# ============================================================================
# Error Input Normalization
# ============================================================================

def normalize_test_errors_to_audit(error_logs: str, target_dir: str) -> Dict[str, Any]:
    """
    Convert raw test error logs into audit report format.
    
    Args:
        error_logs: Raw pytest output with test failures
        target_dir: Directory being tested
        
    Returns:
        Audit-compatible report with issues extracted from test logs
    """
    import re
    from pathlib import Path
    ...
    # Parse pytest failure patterns with detailed extraction
    # Pattern: FAILED test_file.py::TestClass::test_method
    failure_pattern = r"FAILED\s+([\w/\\\.]+\.py)::([\w:]+)"
    
    # Parse assertion errors with actual vs expected
    # Pattern: "assert X == Y" or similar
    assertion_pattern = r"assert\s+(.+?)\s*(==|!=|>|<|>=|<=)\s*(.+)"
    
    # Extract failure details
    failures_found = list(re.finditer(failure_pattern, error_logs))
    
    for match in failures_found:
        test_file_path = match.group(1)
        test_name = match.group(2)
        
        # Infer target file from test file name
        # test_buggy_test.py → buggy_test.py
        # test_module.py → module.py
        target_file = test_file_path.replace("test_", "", 1) if test_file_path.startswith("test_") else test_file_path
        
        # Extract context around this failure
        test_start = match.start()
        context_end = min(test_start + 500, len(error_logs))
        failure_context = error_logs[max(0, test_start-200):context_end]
        
        # Extract specific assertion that failed
        assertion_match = re.search(assertion_pattern, failure_context)
        error_detail = ""
        
        if assertion_match:
            left_side = assertion_match.group(1).strip()
            operator = assertion_match.group(2)
            right_side = assertion_match.group(3).strip()
            error_detail = f"Assertion failed: {left_side} {operator} {right_side}"
        else:
            # Look for error types
            error_type_match = re.search(r"(AssertionError|AttributeError|TypeError|ValueError|NameError):\s*(.+?)(?:\n|$)", failure_context)
            if error_type_match:
                error_detail = f"{error_type_match.group(1)}: {error_type_match.group(2).strip()}"
            else:
                error_detail = f"Test {test_name} failed"
        
        issue_desc = f"Test failure in {test_name}: {error_detail}"
        ...
        # Group by target file (not test file) - use nested dict for compatibility
        if target_file not in file_results:
            file_results[target_file] = {
                "analysis": {"issues": []},
                "issues": [],  # Also add top-level for compat
                "tests_failed": 0,
                "error": None
            }
        file_results[target_file]["analysis"]["issues"].append(issue_desc)
        file_results[target_file]["issues"].append(issue_desc)  # Compat
        file_results[target_file]["tests_failed"] += 1
...
class CorrectorAgent(BaseAgent):
    ...
    def generate_correction(self, 
        ...
        # For test failures, use specialized LLM prompt
        is_test_failure = "test failure" in issue.lower() or "assertion failed" in issue.lower()
        
        if is_test_failure:
            # Use specialized test failure correction
            corrected_code, changed = self._apply_llm_fix_for_test_failure(code, issue)
            strategy = "LLM (Test Failure)"
        else:
            preview = truncate_code(code, max_lines=20)
            prompt = self.prompt_manager.format("corrector_fix", issue=issue, preview=preview)
            
            # Choose correction strategy for non-test issues
    ...
    def _apply_correction_logic(self, code: str, issue: str) -> Tuple[str, bool]:
        """
        Apply AST-based or LLM-based corrections.
        ...
        # For test failures, use LLM-based analysis
        if "test failure" in issue.lower() or "assertion failed" in issue.lower():
            try:
                corrected, changed = self._apply_llm_fix_for_test_failure(code, issue)
                if changed:
                    return corrected, True
            except Exception as e:
                self.logger.debug(f"LLM fix failed for test failure: {e}")
                # Fall through to pattern-based fixes
    ...
    def _apply_llm_fix_for_test_failure(self, code: str, issue: str) -> Tuple[str, bool]:
        """
        Use LLM to generate fixes for test failures.
        
        Args:
            code: Original code
            issue: Issue description (test failure info)
        ...
        try:
            ...
            prompt = f"""Fix this bug based on the test failure:
...
Analyze the test failure and fix ONLY the broken function/logic. Return the COMPLETE corrected code, not just the fixed function.

Rules:
1. Only change what's necessary to fix the specific failure
2. Do NOT change working code
3. Return complete Python code, ready to use
4. If the failure mentions specific assertions, prioritize fixing those"""
            
            response = call_llm(prompt, model="gemini-1.5-flash")
            ...
            # Verify we got valid Python
            try:
                compile(fixed_code, "<string>", "exec")
                return fixed_code, True
            except SyntaxError:
                self.logger.warning("LLM generated invalid Python code")
                return code, False
                
        except Exception as e:
            self.logger.debug(f"LLM-based fix failed: {e}")
            return code, False
    
    def _normalize_error_input(self, error_input: Any, target_dir: str = ".") -> Dict[str, Any]:
        """
        Normalize error input (audit report OR test logs) into standard format.
        
        Args:
            error_input: Dict (audit report), str (test logs), or None
            target_dir: Target directory for context
            
        Returns:
            Standardized audit report format
        """
        if error_input is None:
            # Fresh analysis - return empty report
            return {
                "all_issues": [],
                "file_results": {},
                "total_issues": 0,
        ...
        if isinstance(error_input, dict):
            # Already in audit format - validate and return
            validate_audit_report(error_input)
            return error_input
        
        if isinstance(error_input, str):
            # Raw test error logs - convert to audit format
            return normalize_test_errors_to_audit(error_input, target_dir)
        
        raise ValueError(f"Invalid error_input type: {type(error_input)}. "
                        "Expected dict, str, or None.")
    
    def execute(self, 
                target_dir: str, 
                error_input: Any = None) -> CorrectionSummary:
        """
        Generate corrections based on audit report OR test error logs.
        
        Args:
            target_dir: Path to code directory
            error_input: Either:
                - Dict from AuditorAgent (structured audit report)
                - String from JudgeAgent (raw test error logs)
                - None (analyze directory fresh)
        ...
        # Normalize error_input into audit_report format
        audit_report = self._normalize_error_input(error_input, target_dir)
